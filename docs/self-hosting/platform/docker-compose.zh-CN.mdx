...
- `JWKS_KEY`：LobeHub 签发内部 JWT 使用的 RS256 密钥，网关用其公钥部分校验浏览器会话。每个部署都必须使用自己的密钥。
...
<Steps>
  ...
  如果 `.env` 中的 `JWKS_KEY` 仍是旧版 `.env.example` 自带的值（其 `kid` 为 `6823046760c5d460`），任何人都能伪造你的部署会接受的令牌。请使用 [必要配置](#必要配置) 中的命令生成新密钥并替换，替换后桌面端和 CLI 可能需要重新登录。
