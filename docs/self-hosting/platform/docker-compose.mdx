...
<Callout type="warning">
  **Upgrading an existing Docker Compose deployment?** The latest `docker-compose.yml` adds a gateway service for Gateway Mode. Update `.env` as described in [Upgrading an Existing Deployment](#upgrading-an-existing-deployment) before starting it. Otherwise the gateway container keeps restarting, agent runs fall back to the browser, and `docker logs lobehub` prints an `ACTION REQUIRED` warning.
</Callout>
...
<Callout type="info">
  **System Compatibility Notes**
  ...
  - Port occupation check: Ensure that ports `3210`, `9000`, `9001`, `8787`, and `8788` are available.
</Callout>
...
<Steps>
  ...
  - Domain setup for the Agent Gateway: `gateway.example.com` (the reverse proxy must allow WebSocket upgrades)
  ...
  <Callout type="tip">
    ...
    **Port Mode vs Domain Mode:**
    ...
    - Port Mode: The script automatically uses default ports (3210/9000/9001/8787/8788)
...
`S3_ENDPOINT` only needs to be reachable from browsers: `docker-compose.yml` sets `S3_INTERNAL_ENDPOINT=http://rustfs:9000`, so the LobeHub server itself reaches RustFS inside the Compose network.
...
4. Agent Gateway (Gateway Mode)

The `gateway` service lets agent runs execute on the LobeHub server instead of in the browser. It needs the following values in `.env`, which `setup.sh` fills in automatically:

- `AGENT_GATEWAY_URL`: the gateway address as reached by the browser, such as `https://gateway.example.com` or `http://your_server_ip:8787`. The browser opens a WebSocket connection to it, so a reverse proxy in front of it must allow WebSocket upgrades and long read timeouts. The LobeHub server reaches the gateway at `http://gateway:8787` inside the Compose network.
- `GATEWAY_SERVICE_TOKEN`: a random token shared by LobeHub and the gateway.
- `JWKS_KEY`: the RS256 key set LobeHub signs internal JWTs with; the gateway verifies browser sessions with its public half. Every deployment must use its own key.

```sh
# GATEWAY_SERVICE_TOKEN
openssl rand -hex 32
# JWKS_KEY
docker run --rm --entrypoint /bin/node lobehub/lobehub -e 'const c=require("crypto");const {privateKey}=c.generateKeyPairSync("rsa",{modulusLength:2048});console.log(JSON.stringify({keys:[{...privateKey.export({format:"jwk"}),alg:"RS256",kid:c.randomBytes(8).toString("hex"),use:"sig"}]}))'
```

<Callout type="warning">
  Upgrading an existing deployment? Follow [Upgrading an Existing Deployment](#upgrading-an-existing-deployment) first. The gateway keeps its state in memory, so runs in progress are interrupted when it restarts.
</Callout>

<Callout type="tip">
  Devices joined with `lh connect --gateway http://your_server_ip:8788` can run local tools for this deployment through the device gateway on port `8788`.
</Callout>

## Upgrading an Existing Deployment

<Callout type="warning">
  Finish these steps before starting the new `docker-compose.yml`. Until they are done, the `lobe-gateway` container keeps restarting, agent runs fall back to the browser, and `docker logs lobehub` prints an `ACTION REQUIRED` warning at startup.
</Callout>

<Steps>
  ### Back up `.env` and download the new Compose file
  ...
  ### Add the gateway variables to `.env`

  ```env
  AGENT_GATEWAY_PORT=8787
  DEVICE_GATEWAY_PORT=8788
  # Gateway address as reached by the browser, e.g. https://gateway.example.com
  ...
  In domain mode, also reverse proxy the gateway domain to port `8787` with WebSocket upgrades enabled.

  ### Replace the example `JWKS_KEY`

  If `JWKS_KEY` in `.env` is still the value shipped in the old `.env.example` (its `kid` is `6823046760c5d460`), anyone can forge tokens your deployment accepts. Generate a new key with the command in [Necessary Configuration](#necessary-configuration) and replace it. Desktop and CLI clients may need to sign in again afterwards.
